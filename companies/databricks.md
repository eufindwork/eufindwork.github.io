# Databricks Amsterdam 实习情报

> 更新时间: 2026-05-26
> 信息丰富度: ⭐⭐⭐⭐ (北美面经极多, Amsterdam intern 也有多份真实报告)
> 公司背景: Apache Spark 创始团队创立, AI/data lakehouse 平台龙头, **2024 估值 $62B (pre-IPO)**, Amsterdam 是 EMEA 工程主力 hub

---

## 1. 基本信息

- **公司全称**: Databricks, Inc.
- **HQ**: San Francisco (160 Spear Street)
- **Amsterdam Office**: 阿姆斯特丹 (Zuidas 区), Databricks EMEA 最大工程 hub
- **欧洲其他工程**: Berlin (Germany), Belgrade (Serbia, 工程 hub)
- **其他主要工程**: Mountain View, Bellevue (Seattle), Bangalore
- **业务**: Lakehouse Platform (Spark + Delta Lake + MLflow + Unity Catalog), Mosaic AI, SQL Analytics
- **估值**: pre-IPO 独角兽, 2024 H1 估值约 **$62B** (有 IPO 预期)
- **实习岗位类型 (Amsterdam)**:
  - **Software Engineering Intern (2026)** — 12 周暑期, Amsterdam 招收
  - **Product Management Intern (2026)** — 12 周暑期, Amsterdam 或 Berlin
  - Data Engineer / Data Scientist Intern (偶有, 不固定)
  - ML / Mosaic AI Intern (偶有)
- **实习时长**: 标准 **12 周暑期** (summer, May–Aug); 部分 thesis intern 可延伸到 6 个月
- **申请窗口**: 通常 **8–10 月开放**, 11–12 月开始安排面试, 1–2 月发 offer (北美时间表); EU 时间表略晚, 10–11 月开放
- **资格**: Undergrad / Grad CS 或相关, **预期 fall 2026 – summer 2027 之间毕业** (即 2026 暑期实习面向最后一学年学生)

---

## New Grad / Junior 岗位

> 信息丰富度: ⭐⭐⭐⭐ (Databricks New Grad SDE 是欧洲最炙手可热的 entry-level offer 之一; Levels.fyi Greater Amsterdam L3 直读数据 + Blind RSU 讨论 + IPO 准备最新动态完整)
> 区别于实习: 实习 12 周 + 优先 NL 院校 / EU 学生; **New Grad SWE** 全职 Kennismigrant + 30% ruling, 国际生全方位 sponsor

- **常见岗位名**: **Software Engineer (New Grad) — University Graduate** / **Software Engineer, IC1 / L3** / **SWE I**; Databricks 内部 level **L3 = New Grad (0 yr)**, **L4 = Junior (1-2 yr)**; 没有专门 "Graduate Programme" 品牌, 走标准 SWE + new-grad 蓄水池
- **欧洲地点**:
  - **Amsterdam (Zuidas)**: EMEA 最大工程 hub, New Grad 主要地点, 招 ~30-50 人/cohort (Mosaic AI / SQL / Unity Catalog / Delta Lake teams)
  - **Berlin (DE)**: 次大欧洲 hub, New Grad 名额开始增长 (2024-2025 起)
  - **Belgrade (RS)**: 工程 hub, New Grad 招本地 / 周边国家
  - **London (UK)**: 部分 GTM 加少量 Engineering New Grad
  - 与实习地点一致, 但 New Grad 比实习更倾向 Amsterdam (实习 Berlin 也接, New Grad 主推 NL)
- **是否有独立 Graduate Programme**: **No** — Databricks 没有 cohort-based grad scheme; 走标准 University Recruiting Pipeline (与 Google / Meta / Stripe 类似); 内部有 4 周 onboarding + bootcamp + mentor 制度
- **非EU签证 (全职)**:
  - Databricks Netherlands B.V. 是 IND **Recognised Sponsor**, Kennismigrant 经验成熟 (近 2-3 年快速建立)
  - Amsterdam New Grad SDE **完整 sponsor**: visa + relocation lump sum ($10K-$15K reported on Blind) + 60 天 corporate apartment + 单程机票 + 配偶协助
  - **30% ruling (NL 2026)**: New Grad base **远超** Kennismigrant 阈值 **€4,357/月 <30 岁 = €52.3K/年** [来源 (实际打开过): https://www.jobbatical.com/blog/netherlands-highly-skilled-migrant-salary-thresholds-2026], 100% 适用
  - **对国际生 friendly 度 EMEA 顶级** — 与 Stripe / Snowflake 同档, Databricks 是少数从北美 / 印度 / 中国 top 校园直接 sponsor Amsterdam New Grad 的公司
- **薪资范围 (base, gross/年, 2026-05 Levels.fyi 实读)**:
  - **Greater Amsterdam L3 (New Grad)**: **$86.6K base + $44.8K stock + $6.8K bonus ≈ $138K total / 年 (≈ €119K-€132K total)** [来源 (实际打开过): https://www.levels.fyi/companies/databricks/salaries/software-engineer/locations/greater-amsterdam-area]; TechPays Amsterdam new grad 2 数据点平均 **€132.5K total** [来源 (搜索结果): https://techpays.com/europe/netherlands/databricks/amsterdam/entry-level]
  - **NL L3 (全国 filter)**: **€79.8K-€152K range** [来源 (搜索结果): https://www.levels.fyi/companies/databricks/salaries/software-engineer/levels/l3/locations/netherlands]
  - **Greater Amsterdam 全 level range**: **€118K (L3) - €498K (L6)** [来源 (实际打开过): https://www.levels.fyi/companies/databricks/salaries/software-engineer/locations/greater-amsterdam-area]
  - **Berlin New Grad SDE**: 与 Amsterdam 接近, ~€75K-€90K base + RSU
  - **Belgrade New Grad SDE**: ~€30K-€45K base (当地极顶薪资) + RSU
  - **股权重大变化 (2025 update — 关键)**: Databricks **2025 年取消了 RSU 的 second trigger** — 即便 IPO 未发生, 已 vested 的 RSU 也会直接转 share, 不再需要 IPO/acquisition liquidity event 触发; 这意味着新签 offer **不再是传统 double-trigger pre-IPO 模型** [来源 (搜索结果): https://www.teamblind.com/post/databricks-rsu-value-after-100b-valuation-8rgfsw4j , https://framer.caplight.com/insights/how-databricks-and-stripe-are-redefining-rsu-liquidity-pre-ipo]
  - **RSU vest**: 4 年 **40-30-20-10** schedule (Y1 加速) [来源 (实际打开过): https://www.levels.fyi/companies/databricks/salaries/software-engineer/locations/greater-amsterdam-area]
  - **IPO 状态 (2026 update)**: Databricks **尚未 IPO**, 但 CEO Ali Ghodsi 2025-12 不排除 2026 上市; S-1 预计 2026 H2 提交, 估值已涨到 **$134B (2026)** [来源 (搜索结果): https://www.cnbc.com/2026/01/23/databricks-obtains-1point8-billion-in-additional-debt-ahead-of-ipo.html , https://tech-insider.org/databricks-134-billion-ipo-enterprise-software-2026/]; Revenue run rate ~$4.8-5B 增 55% YoY
  - 30% ruling 后 Amsterdam L3 实得 ≈ €5.5K-€6.5K/月 net + 长期 RSU 兑现 (现在已能 settle)
- **申请窗口**: **每年 8-10 月开放 New Grad SWE** (与北美同步), 11 月 - 次年 2 月面试, 3-5 月发 offer, 7-9 月入职; 错过窗口几乎只能等下一年; New Grad 名额一旦关闭基本无法重开
- **面试流程差异 vs 实习**:
  - 流程结构相同 (HR → Karat → Hiring Manager → Onsite), 但**Onsite 从 4 轮升到 5-6 轮**: +1 **System Design** (45-60 min, 必考: 设计 distributed query engine / 设计 metadata catalog / Spark SQL optimizer) + +1 deep behavioral (与 Director 级 Bar Raiser)
  - **Karat Phone Screen 难度上调** — 实习 1 easy + 1 medium, New Grad 1 medium + 1 hard (典型: graph + DP)
  - **Take-home (偶有)**: 设计 mini distributed cache / mini SQL parser, 5-7 天提交
  - 总流程 6-9 周 (实习 5-7 周)
- **录取竞争**: Amsterdam New Grad SDE 录取率 **~1-2%** (申请池 5,000-8,000/cohort), 是欧洲 entry-level 最难入门之一 (与 Stripe / Citadel / Two Sigma 同档); **国际生 friendliness 高于 Booking / Adyen** (Databricks 全球 hire 文化, 从北美校园直推欧洲 office)
- **关键链接**:
  - Databricks University Careers (筛 EMEA): https://www.databricks.com/company/careers/university-recruiting
  - 全球 Career 总入口 (筛 New Grad + Amsterdam): https://www.databricks.com/company/careers/open-positions
  - Levels.fyi Databricks SWE Greater Amsterdam (2026-05 实读 L3 $138K total): https://www.levels.fyi/companies/databricks/salaries/software-engineer/locations/greater-amsterdam-area
  - TechPays Databricks Amsterdam entry-level: https://techpays.com/europe/netherlands/databricks/amsterdam/entry-level
  - Databricks RSU 取消 second-trigger 讨论 (Blind): https://www.teamblind.com/post/databricks-rsu-value-after-100b-valuation-8rgfsw4j
  - Databricks IPO 2026 准备报道 (CNBC): https://www.cnbc.com/2026/01/23/databricks-obtains-1point8-billion-in-additional-debt-ahead-of-ipo.html
  - LeetCode Discuss Databricks Amsterdam 面经: https://leetcode.com/discuss/interview-experience/?query=databricks+amsterdam
  - 1point3acres Databricks tag: https://www.1point3acres.com/bbs/tag/databricks-1280-1.html
  - Blind databricks 板块 (薪资众包): https://www.teamblind.com/company/Databricks

---

## 2. 签证与国际学生政策

- Databricks NL 是 IND **recognised sponsor**, 全职 SWE 转正全额支持 Kennismigrant + 30% ruling
- **实习生**:
  - EU/EEA → 直接申请, 无需签证
  - 已在 NL 院校就读非 EU → 学习居留 + Nuffic 实习协议 (Databricks 经常用)
  - 不在 EU 院校的非 EU → 需 GVVA, Databricks **支持但优先长期 thesis intern**; 12 周短期暑期对非 EU 海外学生 sponsor 意愿较低
- **Relocation**: Amsterdam intern offer 通常含 **单程机票 + corporate apartment / housing stipend** (12 周项目设计就是 self-contained); 具体数字未公开
- **30% ruling**: 实习生不适用; 全职转正 Amsterdam SWE I 起薪通常超门槛, 适用

---

## 3. 面试流程

Databricks 流程**长且严**, 是榜单中**接受率最低**的公司之一 (与 Stripe / Citadel / 顶级 quant 同级)。

### Amsterdam SE Intern 标准流程 (5–7 周, 4–5 轮)
1. **HR / Recruiter Call** (30 分钟)
   - 经历 + 动机 + "Why Databricks" (**必答**, 期待具体 — Spark? Delta? MLflow? Mosaic?)
   - 项目深入, 资历核查
2. **Karat Phone Screen** (60 分钟, **外包给 Karat 平台**)
   - 通常 **2 题 (1 easy + 1 medium)**, 部分报告会出 3 题但时间不够
   - **不能用 AI / Google** (Karat 是带录像的真人监考)
   - 题型: 数组 / hashmap / linked list / tree, **干净代码 + 测试用例思维**优先于通过 cases
3. **第二轮技术算法** (60 分钟, virtual, 现场写代码 + 高级 + 实习生联合面)
   - LeetCode medium 范围, 偏 graph / DP / sliding window
   - **必带 follow-up**: 优化时间空间, 写测试, 讨论 edge case
   - 部分组会问基础知识 (hash table 内部实现, generic, OOP 概念)
4. **Behavioral / Hiring Manager Round** (45 分钟)
   - "Why Databricks?" "Tell me about Databricks values"
   - 经典: "What negative feedback have you received?" (一份 2025-10 报告原文)
   - 项目深入 + 自我反思 + culture fit
5. **(偶有) 第二轮 manager / 额外技术** — 视组而定, intern 不总有

### Full-time / Senior 对比 (供参考)
- 全职流程: HR + 2 phone screens + **5 onsite rounds** (~9–10 小时), 比 intern 长不少
- onsite 含: 2 coding (LeetCode medium/hard), 1 system design, 1 deep dive (Spark/distributed), 1 behavioral
- 反馈: "比 Facebook / Google 更难, hiring manager 明确说 'looking for the best'"

### 题型重点
- **算法**: LeetCode medium 为底, 但要求**写得干净 + 一次过 + 自带测试**
- **真实考过**: Topological sort for job graph (2025-10 报告), LeetCode 经典图题
- **System Design (intern 偶有, 全职必有)**:
  - 设计一个简化版 Spark scheduler
  - 设计 distributed file system metadata
  - 设计 Delta Lake 概念 (transaction log + parquet)
- **Spark/Distributed 内部 (后期轮次)**:
  - Spark shuffle 原理, partitioning, broadcast vs shuffle join
  - RDD vs DataFrame vs Dataset
  - 容错: lineage, checkpoint
- **行为面 (Databricks 8 Behaviors)**: ownership, customer obsession, take risks, raise the bar

---

## 4. 真实面经

| 时间 | 岗位/城市 | 流程概述 | 题目/题型 | 结果 | 来源 |
|------|----------|---------|----------|------|------|
| 2025-10-28 | SWE Intern / Amsterdam | HR + 2 algo + manager | **Topological sort on job graph**; behavioral: "negative feedback received", Databricks values | **Accepted Offer (Positive)** | [JoinTaro](https://www.jointaro.com/interviews/companies/databricks/experiences/software-engineer-intern-amsterdam-october-28-2025-accepted-offer-positive-cd02b72d/) |
| — | SE Intern / Amsterdam | Karat + tech + behavioral + (额外 tech) | Karat: 1 easy + 1 medium; test suite follow-up | — | [Blind](https://www.teamblind.com/post/databricks-amsterdam-se-intern-interview-process-experience-neuk5z7j) |
| 2024-01 | Engineer Manager / Amsterdam | 多轮 + leadership | Heavy on distributed systems + Spark internals | No Offer (Negative) | [JoinTaro](https://www.jointaro.com/interviews/companies/databricks/experiences/engineer-manager-amsterdam-january-15-2024-no-offer-negative-77663311/) |
| 2025-09 | Senior SWE Manager / Amsterdam | 多轮 leadership + system | Distributed + leadership | Offer | [JoinTaro](https://www.jointaro.com/interviews/companies/databricks/experiences/senior-software-engineering-manager-amsterdam-september-1-2025-accepted-offer-positive-4f7c4b67/) |
| 2025-08 | Engineering Manager / Amsterdam | 多轮 | — | No Offer (Negative) | [JoinTaro](https://www.jointaro.com/interviews/companies/databricks/experiences/engineering-manager-amsterdam-august-1-2025-no-offer-negative-5b50891f/) |
| — | Intern (general) | Karat → tech → behavioral | LeetCode medium, 写测试 | — | [Blind 综合](https://www.teamblind.com/post/databricks-intern-interview-41nvpuk4) |
| — | SDE (Blind 综合) | "至少 9–10 小时, 比 Google 难" | LeetCode medium/hard, system design | Many no offer | [Blind 综合](https://www.teamblind.com/post/Is-Databricks-worth-going-through-their-LONG-interview-loop-bf184MAw) |

### 真实题型 highlights
- **Topological sort 图题** (2025-10 Amsterdam intern 真题): "ordering jobs with dependencies"
- **Karat 高频**: 数组 + hashmap (LeetCode 1, 49, 128, 200 系列)
- **Behavioral 必备**: 
  - "Why Databricks specifically (vs Snowflake / DBT / Confluent)?"
  - "What negative feedback have you received?" 
  - "Tell me about a time you owned something hard"

---

## 5. Offer 案例 / Stipend

| 城市 | Stipend (gross/月) | 备注 | 时间 | 来源 |
|------|------|------|------|------|
| Amsterdam | TechPays Avg: **€92,000/年** intern 总包 (数据点少) | Tech intern | 2024 | [TechPays](https://techpays.com/europe/netherlands/databricks/amsterdam) |
| Amsterdam | 估算 **€3,000–€4,500/月 gross** + 住房安排 | SWE intern | 2024–2025 | 多源综合 |
| US (intern 对比) | $54.50/小时 (~$9,400/月) | SF / Bellevue | 2024 | [Levels.fyi](https://www.levels.fyi/internships/Databricks/Software-Engineer-Intern/) |
| Amsterdam SWE 全职 (intern 转正参考) | €120K–€180K total (base + RSU + signing) | full-time | 2024 | [Levels.fyi / TechPays](https://techpays.com/europe/netherlands/databricks/amsterdam) |

### 关键备注
- Databricks Amsterdam intern stipend 是**本榜单 top tier**, **远高于 Adyen / ASML / TomTom**, 跟 Booking / Uber 同档或更高
- Amsterdam stipend 比北美低约 50%, 但加上 30% ruling 和 NL 物价, 实际生活水平接近
- pre-IPO RSU 是巨大潜在 upside (如果 IPO 完成估值兑现)
- 12 周项目设计为 self-contained, 住房 / 机票 / 设备一站式

---

## 6. 申请渠道与建议

- **官方校招主页**: https://www.databricks.com/company/careers/university-recruiting
- **职位列表**: https://www.databricks.com/company/careers/open-positions
- **Amsterdam SWE Intern 2026 直链**: https://www.databricks.com/company/careers/university-recruiting/software-engineering-intern-2026-6866534002
- **Amsterdam PM Intern 2026 直链**: https://www.databricks.com/company/careers/product/product-management-intern-2026---amsterdam-8133715002
- **面试准备指南 (官方)**: https://www.databricks.com/company/careers/interview-prep

### 推荐策略
1. **早申**: 8 月底-9 月初投递, 12 周项目竞争极激烈
2. **只能投 1 个 location** (重要): Databricks 明确规定每 30 天只能 1 份申请 — 慎选 city
3. **LeetCode**: 至少 200+ medium, 重点 graph (top sort, BFS/DFS), DP, sliding window, hashmap
4. **Karat 录像监考**: 提前熟悉平台 UI, 摄像头位置, 出声 think-aloud
5. **写测试**: Databricks 极看重 "write a test suite" 能力 — 准备时主动给每道题写 unit test
6. **Spark / Data 背景加分**: 简历有 Spark / Kafka / Delta / Iceberg / dbt 经验, manager 面会聊得很深
7. **"Why Databricks" 必须具体**: 提到具体产品 (Mosaic AI, Unity Catalog, MLflow, DBSQL), 不要泛泛"AI/data"
8. **行为面**: 准备 **"negative feedback"** 故事 (高频出现), 真实 + 反思 + 行动
9. **EU 学生优势**: 不需要签证, 录取概率显著更高; 非 EU 学生**强烈建议持有 NL/EU 学位 + 学习居留**再申请

---

## 7. 踩坑与社区评价

### 警惕点
- **流程极严**: 5–6 轮, 接受率 << 5%, Blind 上多帖抱怨"比 Facebook/Google 都难"
- **Long loop**: full-time onsite 9–10 小时, intern loop 也 5–6 小时虚拟; 时间投入巨大
- **picky 文化**: hiring manager 拒人风格"looking for the best candidate", 没有 mediocre offer
- **题目难度上升**: 2024 后 medium 居多, follow-up 要写测试 + 性能优化 + edge case, 不只是 "AC 即可"
- **不能用 AI 工具**: Karat 监考严格, ChatGPT / Copilot 帮助不到
- **Amsterdam intern 名额少**: 估计每年 < 20 人 cohort (vs 北美几百人)
- **pre-IPO 估值变动**: RSU 价值不确定, 早期员工享受到 IPO upside, intern 享受到的程度依赖入职时间

### 优点
- **品牌顶级**: Databricks 简历背书 = 顶级 AI/data 公司, 跳 Snowflake / Confluent / OpenAI / Anthropic 都加分
- **stipend 极高**: 本榜单最高之一, Amsterdam 实习足够过得很好
- **mentorship 强**: 12 周有明确 project + mentor + 同期 cohort 活动
- **return offer 率高**: 公开报告显示 return rate **70%+** (含 Amsterdam)
- **技术栈 cutting edge**: Spark, MLflow, Mosaic AI (LLM training/serving), Unity Catalog
- **Amsterdam Zuidas 位置**: 现代化办公区, 公共交通方便

### 利与弊
- (+) 顶级技术 + 顶级 stipend + 顶级品牌
- (+) return offer 率高, intern → full-time → senior 路径明确
- (-) 流程难度极高, 准备时间投入大
- (-) Amsterdam 名额少, 非 EU 学生概率更低
- (-) IPO 未发生前 RSU 价值不确定

---

## 8. 关键链接汇总

- 官方校招主页: https://www.databricks.com/company/careers/university-recruiting
- 全部职位: https://www.databricks.com/company/careers/open-positions
- Amsterdam SWE Intern 2026: https://www.databricks.com/company/careers/university-recruiting/software-engineering-intern-2026-6866534002
- Amsterdam PM Intern 2026: https://www.databricks.com/company/careers/product/product-management-intern-2026---amsterdam-8133715002
- Berlin PM Intern 2026 (备选): https://www.databricks.com/company/careers/product/product-management-intern-2026---berlin-8145776002
- 官方面试准备页: https://www.databricks.com/company/careers/interview-prep
- 工程文化博客: https://www.databricks.com/company/careers/engineering-at-databricks
- Engineering Blog: https://www.databricks.com/blog/category/engineering
- JoinTaro 经验集: https://www.jointaro.com/interviews/companies/databricks/
- Blind 实习专门帖: https://www.teamblind.com/post/databricks-amsterdam-se-intern-interview-process-experience-neuk5z7j
- Exponent 流程指南: https://www.tryexponent.com/blog/databricks-interview-process
- Glassdoor 面经: https://www.glassdoor.com/Interview/Databricks-Amsterdam-Interview-Questions-EI_IE954734.0,10_IL.11,20_IM1112.htm
- TechPays Amsterdam: https://techpays.com/europe/netherlands/databricks/amsterdam
- Levels.fyi Databricks SWE Greater Amsterdam (2026-05 实读): https://www.levels.fyi/companies/databricks/salaries/software-engineer/locations/greater-amsterdam-area
- TechPays Databricks Amsterdam entry-level: https://techpays.com/europe/netherlands/databricks/amsterdam/entry-level
- Databricks RSU second-trigger 取消讨论 (Blind 2025-2026): https://www.teamblind.com/post/databricks-rsu-value-after-100b-valuation-8rgfsw4j
- Databricks pre-IPO RSU liquidity redesign (Caplight): https://framer.caplight.com/insights/how-databricks-and-stripe-are-redefining-rsu-liquidity-pre-ipo
- Databricks $134B 估值 IPO 2026 报道: https://tech-insider.org/databricks-134-billion-ipo-enterprise-software-2026/
- Databricks $1.8B debt 2026 IPO 前融资 (CNBC): https://www.cnbc.com/2026/01/23/databricks-obtains-1point8-billion-in-additional-debt-ahead-of-ipo.html
- NL 2026 Kennismigrant threshold: https://www.jobbatical.com/blog/netherlands-highly-skilled-migrant-salary-thresholds-2026
